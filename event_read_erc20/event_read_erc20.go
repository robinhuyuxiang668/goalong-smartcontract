...
func main() {
	...
	for _, vLog := range logs {
		...
		case logTransferSigHash.Hex():
			...
			//序列化数据:注释调这行是因为高版本abi.unpack只有两个参数 ,3个参数只有go-ethereum v1.9.2有，如果data很复杂考虑用1.9.2
			//使用abi.unpack 只能解析data不分,不会解析 topics字段(event indexed修饰的)
